...
The network has incentives to work hard to earn BTC (and thus invest CPU power), since the quality and security of the distributed ledger is directly proportional to their participation in the mining (provision of marginal CPU power);
...
Trust: if you aren't actively mining (and thus making verifying the blockchain independently), then you can re-join the network and take the blockchain provided as gospel.
...
The blockchain is the majority decision representing the longest chain, which by design has the most proof of work / effort in it.

To modify the past, or hack the blockchain, a hacker would need to re-do all the previous work, and keep up with on-going work, in order to beat the combined effort of the honest CPU nodes.
